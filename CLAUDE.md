...
- **`SUBSCRIPTION_JSON_FORMAT=true`** (on the xray-checker container) is load-bearing, not optional: RemnaWave only serves real proxy configs to recognized client User-Agents and silently returns a fake "subscription not found" placeholder to anything else, including xray-checker's own default UA. This flag makes xray-checker impersonate the Happ app (`User-Agent: Happ/1.0`), which RemnaWave does recognize. Without it, the subscription proxy above receives nothing to patch — this cost real debugging time, don't remove it without confirming RemnaWave's UA allowlist another way.
